...
def pedido_command(update, context):
  ...
  if (hora_pedido >= 1 and hora_pedido <= 15): 
    ...
    pedidos = sh.worksheet("Pedidos")
    #Chama o método next_available_row(worksheet) da linha 76 e faz uso da worksheet 'Pedidos'
    next_row = next_available_row(pedidos)
    
    if dia_semana == 1:
      # pega apenas as linhas onde o valor em Dia é Segunda
      dados_almoco.loc[dados_almoco['Dia'] == 'Segunda']
      if fichas_int == 0:
        update.message.reply_text(f"Olá {update.message.from_user['first_name']}.""\nEsse prato não está mais disponível.")
      else:
        pedidos.update_acell("B{}".format(next_row), update.message.from_user['first_name'])
        pedidos.update_acell("C{}".format(next_row), update.message.from_user['last_name'])
        pedidos.update_acell("D{}".format(next_row), prato)
      ...
      dados_almoco.loc[dados_almoco['Dia'] == 'Terça']
      if fichas_int == 0:
        update.message.reply_text(f"Olá {update.message.from_user['first_name']}.""\nEsse prato não está mais disponível.")
      else:
        pedidos.update_acell("B{}".format(next_row), update.message.from_user['first_name'])
      ...
      dados_almoco.loc[dados_almoco['Dia'] == 'Quarta']
      if fichas_int == 0:
        update.message.reply_text(f"Olá {update.message.from_user['first_name']}.""\nEsse prato não está mais disponível.")
      else:
        pedidos.update_acell("B{}".format(next_row), update.message.from_user['first_name'])
      ...
      dados_almoco.loc[dados_almoco['Dia'] == 'Sexta']
      if fichas_int == 0:
        update.message.reply_text(f"Olá {update.message.from_user['first_name']}.""\nEsse prato não está mais disponível.")
      else:
        pedidos.update_acell("B{}".format(next_row), update.message.from_user['first_name'])
    ...
    #dados_almoco["Fichas"] = dados_almoco['Fichas'].replace([7,4],10)
    #dados_almoco.loc[dados_almoco['Fichas'] >= 0]

    dados_almoco['Fichas'] = 10

    update.message.reply_text(f"Olá {update.message.from_user['first_name']}.""\nA cantina ainda não disponibilizou a retirada de fichas.")
  
    sh.values_clear("'YTD'!A2:E3")
